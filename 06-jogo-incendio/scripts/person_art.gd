...
# A fábrica de gente — trazida do Projeto 5 V2, onde ela nasceu, porque o que
# ela resolve não tem nada a ver com colônia nenhuma: montar uma pessoa de
# pixel art a partir de peças e recolorir cada camada sem borrar o sombreado.
#
# Aqui ela serve a duas populações com papéis opostos: os BRIGADISTAS, que
# obedecem a você, e os MORADORES, que fogem sozinhos. Eles precisam ser
# distinguíveis num relance no meio da fumaça — daí `crew_look()` e
# `civilian_look()` logo abaixo do sorteio genérico, que prendem a paleta de
# cada grupo em vez de sortear qualquer cor.
#
...
# ---- expressão a partir do estado de quem está num incêndio ----

# A cara segue o MEDO, que aqui é a única necessidade que importa. `fear` vem
# do perigo da célula onde a pessoa está, então o rosto conta a temperatura do
# lugar antes de o jogador conseguir ler a cor do chão.
static func expression_for_fire(state: String, fear: float, blinking: bool) -> int:
	if state == "lost":
		return EXPR_SLEEP
	if state == "safe":
		return EXPR_HAPPY
...
# ---- paletas por papel ----
#
# Brigadista sempre de camisa clara e chapéu; morador nunca de chapéu e com a
# paleta de roupa aberta. Não é enfeite: numa tela com trinta pessoas correndo
# entre chamas, quem manda e quem foge têm que se separar sem legenda.

const CREW_SHIRT_COLORS := [
	Color("d8dee9"), Color("e8e6df"), Color("c9d1d9"),
]
const CREW_HAT_COLORS := [
	Color("f2b134"), Color("e8a020"),
]

static func crew_look(rng: RandomNumberGenerator) -> Dictionary:
	var look := random_look(rng)
	look["shirt_color"] = CREW_SHIRT_COLORS[rng.randi_range(0, CREW_SHIRT_COLORS.size() - 1)].to_html(false)
	look["hat"] = rng.randi_range(0, HAT_TILES.size() - 1)
	look["hat_color"] = CREW_HAT_COLORS[rng.randi_range(0, CREW_HAT_COLORS.size() - 1)].to_html(false)
	return look

static func civilian_look(rng: RandomNumberGenerator) -> Dictionary:
	var look := random_look(rng)
	look["hat"] = -1
	return look

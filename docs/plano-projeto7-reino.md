...
- **Desbloqueio de prédios por nível. [concluída 2026-08-13]** O corte deliberado registrado na Fase 7 e no `progression.gd` ("gatear isso por nível pediria reescrever a colocação de prédios pra ser progressiva") foi resolvido: `_unlock_building_tier(level)` em `main.gd` planta só os prédios daquele nível, chamada por `_maybe_unlock_next_tier()` todo `_process()` assim que `Progression.level` ultrapassa o que já foi desbloqueado. A vila agora nasce **pequena** (nível 1: só Armazém, Fazenda e uma Casa — sobrevivência antes de economia) e ganha Posto de Lenhador/Pedreira (nível 2), Mina/Serraria (nível 3), Oficina de Pedra/Gerador/Forja (nível 4) — `MAX_BUILDING_TIER = 4` é um teto de verdade, nível 5 só amplia o alcance de exploração. `pathfinder.rebuild()` e a criação de nós da cena, que rodavam uma vez só no frame 1, viraram incrementais (`_jobs_registered_up_to`/`_nodes_built_up_to`) pra suportar prédio novo aparecendo a qualquer momento do jogo.
- **Roda D'Água + Moinho de Vento. [concluída 2026-08-13]** Segunda e terceira fonte de energia do plano original — mesma regra "energia OU trabalhador" do Gerador, mas grátis pra operar depois de construídas (o custo é a posição exigida, não combustível contínuo). `Buildings.POWER_SOURCE_KINDS` generaliza `_compute_powered()` pra qualquer fonte no raio; só `FUELED_POWER_KINDS` (o Gerador) gasta estoque. Roda D'Água só nasce numa célula de grama com água de verdade do lado (`main.gd::_cell_near_water`, lê `WaterSim` de verdade); Moinho de Vento só em terreno alto o bastante (`_cell_on_high_ground`) — precisou de limiar próprio (`WINDMILL_MIN_HEIGHT = 3.0`, medido) porque o limiar de altura que já existia pra "elevado" (`MapGen.HILLS_HEIGHT`) vira Colina, nunca Grama. As duas nascem no nível 5 (o último), que passou a existir como tier de prédios de propósito — `MAX_BUILDING_TIER` subiu de 4 pra 5, reaproveitando toda a infraestrutura de desbloqueio por nível sem mudança de arquitetura nenhuma.

...
- **Desbloqueio de prédios por nível. [concluída 2026-08-13, depois SUBSTITUÍDA — ver item seguinte]** O corte deliberado registrado na Fase 7 e no `progression.gd` ("gatear isso por nível pediria reescrever a colocação de prédios pra ser progressiva") foi resolvido: `_unlock_building_tier(level)` em `main.gd` plantava só os prédios daquele nível, chamada por `_maybe_unlock_next_tier()` todo `_process()` assim que `Progression.level` ultrapassava o que já tinha sido desbloqueado. A vila nascia **pequena** (nível 1: só Armazém, Fazenda e uma Casa — sobrevivência antes de economia) e ganhava Posto de Lenhador/Pedreira (nível 2), Mina/Serraria (nível 3), Oficina de Pedra/Gerador/Forja (nível 4) — `MAX_BUILDING_TIER = 4` era um teto de verdade, nível 5 só ampliava o alcance de exploração. `pathfinder.rebuild()` e a criação de nós da cena, que rodavam uma vez só no frame 1, viraram incrementais (`_jobs_registered_up_to`/`_nodes_built_up_to`) pra suportar prédio novo aparecendo a qualquer momento do jogo — essa parte sobreviveu à mudança seguinte.
- **Roda D'Água + Moinho de Vento. [concluída 2026-08-13]** Segunda e terceira fonte de energia do plano original — mesma regra "energia OU trabalhador" do Gerador, mas grátis pra operar depois de construídas (o custo é a posição exigida, não combustível contínuo). `Buildings.POWER_SOURCE_KINDS` generaliza `_compute_powered()` pra qualquer fonte no raio; só `FUELED_POWER_KINDS` (o Gerador) gasta estoque. Roda D'Água só pode ser construída numa célula de grama com água de verdade do lado (lê `WaterSim` de verdade); Moinho de Vento só em terreno alto o bastante — precisou de limiar próprio (`WINDMILL_MIN_HEIGHT = 3.0`, medido) porque o limiar de altura que já existia pra "elevado" (`MapGen.HILLS_HEIGHT`) vira Colina, nunca Grama.
- **Agência de jogador: menu de construção manual. [concluída 2026-08-14, substitui o desbloqueio por nível]** João pediu pra abrir o jogo e jogar — viu a vila crescendo sozinha (todo prédio automático por nível, nenhuma decisão do jogador) e, perguntado que tipo de agência fazia sentido, escolheu explicitamente "escolher O QUE construir e QUANDO, pagando um custo em recurso acumulado". Isso significou: (1) `Buildings.BUILD_COST` — custo fixo por `Kind`, pago do Armazém; Casa/Lenhador/Pedreira custam só comida (único recurso que existe antes de qualquer economia rodar — a Fazenda continua nascendo de graça, é o piso de sobrevivência), o resto custa madeira/pedra; (2) o sistema de tiers (`_unlock_building_tier`/`_place_tier2..5_buildings`/`_maybe_unlock_next_tier`) foi **removido** — `Progression.level` voltou a só controlar o alcance de exploração, como na Fase 7 original; (3) um painel de construção (`_build_construction_panel`) lista todo prédio comprável com custo, desabilitado sem estoque; clicar arma o modo de construção, um quadrado-fantasma verde/vermelho (`_update_ghost`) mostra se a célula sob o mouse é válida, e o clique confirma (`_build`) — Esc/botão direito cancelam; (4) a lógica de "onde é válido" (depósito certo, água do lado, terreno alto) sobreviveu quase inteira do sistema de tiers, só migrou de "escolhida automaticamente" pra "validada contra o clique do jogador" (`_can_place_kind_at`); (5) construção também respeita a névoa de guerra — não dá pra plantar prédio em área nunca explorada. 297 asserções headless (testes que dependiam do desbloqueio por tempo foram reescritos pra construir manualmente, como um jogador faria).

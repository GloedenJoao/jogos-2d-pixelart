...
const IDLE_WANDER_CHANCE := 0.3   # chance, por avaliação, de um ocioso passear
	...
	match best:
		"eat":
			if v.state != Villager.STATE_EATING:
				_send_to(v, Valley.plaza_spot(rng), "eat")
		"rest":
			if v.state != Villager.STATE_RESTING:
				_send_to(v, Valley.plaza_spot(rng), "rest")
		"work":
		...
		_:
			...
			# em quando dá uma volta pela praça. Sem isto, colônia com folga de
			# gente vira um monte de sprites imóveis e a tela parece congelada.
			elif rng.randf() < IDLE_WANDER_CHANCE:
				_send_to(v, Valley.plaza_spot(rng), "idle")

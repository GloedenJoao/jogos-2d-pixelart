...
## Estado atual: Fazenda — necessidade de comida

Primeira necessidade de população de verdade: cada habitante consome comida por segundo, e a Fazenda é a primeira fonte de produção SEM depósito finito — não existe tile de "terra fértil" no mapa, ela representa lavoura ao redor da vila.
...
- [`tests/run_tests.gd`](tests/run_tests.gd) — 265 asserções: as anteriores, mais consumo de comida, fome encolhendo população até o piso, equilíbrio com comida parcial, e extração da Fazenda sem depósito nenhum, isolados.

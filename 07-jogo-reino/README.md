...
## Estado atual: Fase 3 — transporte

A produção deixou de virar estoque jogável na hora. Cada extrator produz pro próprio PÁTIO (`Building.buffer`, com teto); um NPC carregador precisa fisicamente levar aquilo até o Armazém pra virar recurso de verdade (`Buildings.stock`, o que aparece no HUD).

- [`scripts/carrier.gd`](scripts/carrier.gd) + [`scripts/carriers.gd`](scripts/carriers.gd) — o carregador escolhe sempre o pátio **mais cheio** (não o mais próximo, não por turno fixo — com um carregador só servindo dois extratores, quem está prestes a travar por falta de espaço é atendido primeiro), anda até lá, pega até a capacidade, leva pro Armazém, entrega, decide de novo. Mesmo avanço por posição contínua de `workers.gd`, mas o ciclo de decisão é coisa de carregador — por isso é uma classe separada, não uma extensão de `Worker`.
- [`scripts/buildings.gd`](scripts/buildings.gd) ganhou `Kind.WAREHOUSE` (sem produção própria, só um destino sólido no pathfinder), `Building.buffer` (produção acumulada no pátio) e `EXTRACTOR_BUFFER_CAP` — um extrator com o pátio cheio **para de extrair sozinho** (o trabalhador continua `WORKING`, mas nada sai do depósito até alguém vir buscar). Isso é o que dá ao transporte um motivo de existir: sem teto, "pátio" seria só um segundo nome pra "estoque", e o carregador seria decoração.
- `scenes/main.gd` coloca o Armazém na própria célula da vila (destino fixo, sem busca de depósito) e spawna um carregador. Marcador visual: um quadradinho da cor do recurso aparece acima do carregador só enquanto ele está carregando algo — é o que diferencia "indo buscar" de "voltando com carga" numa olhada, sem animação.
- [`tests/run_tests.gd`](tests/run_tests.gd) — 175 asserções (Fase 0 + 1 + 2 + 3): as anteriores, mais o teto do pátio throttlando extração sem desperdiçar o depósito, `collect`/`deliver`, o carregador escolhendo sempre o pátio mais cheio, o ciclo completo de ida e volta entregando no Armazém, viagens abaixo do mínimo sendo ignoradas, e nenhuma coleta acontecer sem Armazém no mapa.
...
[`tests/visual_capture.gd`](tests/visual_capture.gd) captura vila inicial, os trabalhadores nos postos, o carregador entregando no Armazém, mapa depois de explorar e vista afastada, em `.visual_capture/` (gitignored).

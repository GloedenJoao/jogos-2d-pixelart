...
- [`tests/run_tests.gd`](tests/run_tests.gd) — 98 asserções (Fase 0 + Fase 1): determinismo do mapa, cobertura de cada tipo de depósito numa faixa plausível pra várias sementes, extração/esgotamento/regeneração, segurança fora dos limites, integração com `WaterSim`, névoa revelando em círculo e mantendo memória quando a fonte se move, e testes de cena de verdade (câmera limitada ao mapa, clique revela névoa, segurar tecla move e trava na borda, a cena semeia lago e a cobertura fica numa faixa de "lago", não poça nem inundação).

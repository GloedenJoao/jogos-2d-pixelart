...
## Estado atual: Roda D'Água + Moinho de Vento

Segunda e terceira fonte de energia do plano original (o Gerador a Lenha foi só a primeira) — mesma regra "energia OU trabalhador", mas as duas são **grátis pra operar** depois de construídas, sem consumir combustível nenhum.

- [`scripts/buildings.gd`](scripts/buildings.gd) — `Kind.WATERWHEEL`/`Kind.WINDMILL` entram em `POWER_SOURCE_KINDS` (junto com o Gerador, mesmo `GENERATOR_RADIUS`), mas NÃO entram em `FUELED_POWER_KINDS` — `_compute_powered()` só cobra combustível de quem está nessa segunda lista. Zero mudança na lógica de produção (`advance()`/`_advance_extractor()`/`_advance_processor()`), só generalizou de "olha só pro Gerador" pra "olha pra qualquer fonte na lista".
- **O "custo" de cada fonte é a posição, não manutenção contínua** — [`scenes/main.gd`](scenes/main.gd) só planta Roda D'Água numa célula de grama com água de verdade do lado (`_cell_near_water`, lê `water_sim.water_at()` de verdade, limiar bem acima do que já é visível na tela) e só planta Moinho de Vento em terreno alto o bastante (`_cell_on_high_ground`, `map.height_at() >= WINDMILL_MIN_HEIGHT`) — precisou de um limiar PRÓPRIO (3.0, medido rodando o mapa real) porque o limiar que já existia pra "alto" (`MapGen.HILLS_HEIGHT`) vira Colina, não Grama, então nenhuma célula de grama jamais bateria aquele limiar. Sem célula válida perto da vila, aquele prédio simplesmente não nasce — mesma regra de "explore mais" dos extratores de depósito.
- **Nível 5 (o último tier) é onde as duas nascem** — `MAX_BUILDING_TIER` subiu de 4 pra 5, reaproveitando 100% da infraestrutura de desbloqueio por nível que já existia (ver "Fases anteriores" abaixo); nenhuma peça nova de arquitetura, só mais uma chamada em `_unlock_building_tier`.
- [`tests/run_tests.gd`](tests/run_tests.gd) — 288 asserções: as anteriores, mais Roda D'Água e Moinho de Vento sustentando produção sem gastar NENHUM estoque (isolados, provando a ausência de custo continuo), e um teste de cena confirmando que a semente real do jogo tem água/terreno alto perto o bastante da vila pros dois nascerem de verdade.

## Fases anteriores

**Desbloqueio de prédios por nível.** Até aqui todo prédio nascia de uma vez no frame 1 — corte deliberado registrado desde a Fase 7 ("gatear isso por nível pediria reescrever a colocação de prédios pra ser progressiva, mudança de arquitetura maior que o resto desta fase"). `Progression.level` (progression.gd) passou a controlar o quê existe: a vila nasce **pequena** (só Armazém + Fazenda + uma Casa) e ganha prédios novos a cada nível.

- [`scenes/main.gd`](scenes/main.gd) — `_unlock_building_tier(level)` planta só os prédios daquele nível (`_place_tier1_buildings` … `_place_tier5_buildings`); `_maybe_unlock_next_tier()`, chamado todo `_process()`, dispara o próximo tier assim que `progression.level` ultrapassa `_unlocked_level`. Nível 1: Armazém, Fazenda, uma Casa (só sobrevivência). Nível 2: Posto de Lenhador, Pedreira, segunda Casa. Nível 3: Mina, Serraria, terceira Casa. Nível 4: Oficina de Pedra, Gerador, Forja. Nível 5: Roda D'Água, Moinho de Vento (ver "Estado atual" acima) — fecha o catálogo de vez.

...
## Estado atual: Fase 2 — extração básica + um trabalhador por prédio

Primeiro ciclo econômico de verdade: cada prédio produz, um trabalhador precisa estar lá pra isso acontecer, e o depósito que ele explora é o mesmo da Fase 1 — sem contabilidade duplicada.
...
- [`scripts/buildings.gd`](scripts/buildings.gd) — prédio extrator senta EM CIMA de uma célula de depósito do `MapGen` e puxa direto dali (`map.extract()`) — sem estoque próprio de matéria-prima duplicado. Só produz com o trabalhador de fato `WORKING` (chegou e está lá), não só "alocado": o trajeto até o posto custa tempo de produção de propósito, é o que o plano do projeto pede ("prédio longe do núcleo populacional custa tempo de trajeto"). Staffing é ligado/desligado, um trabalhador por prédio — o mecanismo de fração de vaga ocupada da Colônia V2 (`staffing_ratios()`) só vale a pena quando houver múltiplas vagas por prédio.
- `scenes/main.gd` coloca um Posto de Lenhador e uma Pedreira na célula de depósito mais próxima da vila (busca em anéis crescentes — mapa sem aquele recurso por perto simplesmente não ganha aquele prédio agora, não é erro) e spawna **um trabalhador por prédio** (`for i in buildings.list.size()`), cada um andando até o seu posto e produzindo ao chegar. A primeira versão desta fase nascia com um único trabalhador no total — o suficiente pra provar o sistema, mas deixava a Pedreira sempre parada, e um extrator parado é indistinguível de um extrator que não existe pra quem só está jogando. Trabalhador é um sprite do pack Kenney "Roguelike Characters" — um rosto por trabalhador, não clone (mesma técnica simples do `05-jogo-colonia`, não o paper doll modular da V2) — com uma bolinha de estado colorida acima da cabeça; prédio é um retângulo com "telhado" e borda escura gerados por código (sem sprite de construção isolada nos acervos disponíveis, e a borda existe porque a primeira versão da Pedreira usava cinza — quase a mesma cor do próprio depósito de pedra por baixo dela, virando camuflagem em vez de prédio).
- [`tests/run_tests.gd`](tests/run_tests.gd) — 145 asserções (Fase 0 + 1 + 2): as anteriores, mais pathfinder (rota em grade aberta, desvio de sólido, trilha se formando e decaindo), trabalhadores (chegada exata no alvo, trabalhadores independentes) e prédios (só produz com trabalhador `WORKING`, produção para sozinha quando o depósito esgota, busca do depósito mais próximo, todo prédio nasce com trabalhador — nenhum fica parado).
...
[`tests/visual_capture.gd`](tests/visual_capture.gd) captura vila inicial, os dois trabalhadores nos postos, mapa depois de explorar e vista afastada, em `.visual_capture/` (gitignored).

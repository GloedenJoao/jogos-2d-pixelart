...
## Estado atual: Fase 6 — população

Mão de obra deixa de ser infinita e instantânea. Até a Fase 5, todo trabalhador e o carregador nasciam prontos no primeiro frame; agora existe uma **população** que cresce devagar até a capacidade habitacional (soma das Casas construídas), e prédios só ganham gente conforme ela cresce.

- [`scripts/population.gd`](scripts/population.gd) — `count` cresce a `GROWTH_PER_SECOND` até `housing_capacity`, nunca passa do teto. `available()` é quanto ainda não foi empregado; `employ()` reserva um. Deliberadamente NÃO modela necessidades (comida, água potável, descanso) — o plano do projeto pede isso também na Fase 6, mas exigiria uma fonte de comida (Fazenda/Posto de Caça) que nenhuma fase anterior construiu.
- [`scripts/buildings.gd`](scripts/buildings.gd) ganhou `Kind.HOUSE` e `housing_capacity()` — a Casa não produz nada, só soma capacidade; é a `Population` quem lê esse número.
- `scenes/main.gd`: prédios que precisam de trabalhador entram numa fila (`_pending_jobs`) em vez de ganhar um NPC na hora; `_fill_jobs()` em `_process()` preenche a fila (e o carregador) conforme `population.available()` permite. Ordem da fila é prioridade: extratores antes da Serraria (que depende da produção deles chegar no Armazém primeiro), carregador por último.
- [`tests/run_tests.gd`](tests/run_tests.gd) — 232 asserções (Fase 0 + 1 + 2 + 3 + 4 + 5 + 6): as anteriores, mais população crescendo até o teto sem ultrapassar, capacidade zero (sem Casa) travando o crescimento, `employ()`/`available()` respeitando o limite, e a cena real mostrando a vila vazia no primeiro frame e progressivamente povoada até todo prédio staffável ganhar trabalhador.
...
[`tests/visual_capture.gd`](tests/visual_capture.gd) captura a vila **vazia** no primeiro frame (população 0/6), depois **povoada** conforme a população cresce, o carregador entregando no Armazém, a cadeia de processamento (com a Oficina de Pedra produzindo só por energia), mapa depois de explorar e vista afastada, em `.visual_capture/` (gitignored).

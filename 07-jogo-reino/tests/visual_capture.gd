...
func _initialize() -> void:
	...
	# Nível 1: a vila nasce PEQUENA de propósito agora (Armazém + Fazenda +
	# uma Casa, ver `_unlock_building_tier` em main.gd) — nada de Posto de
	# Lenhador, Pedreira, Mina, Serraria etc. até a vila subir de nível.
	await _capture("01_vila_inicial", main)

	# Avança até o último tier desbloquear (nível 4) — só depois disso o
	# catálogo de prédios do jogo está completo, e as capturas seguintes
	# fazem sentido mostrar todo mundo de uma vez.
	var unlock_steps := 0
	while unlock_steps < 40000 and main._unlocked_level < main.MAX_BUILDING_TIER:
		main._process(1.0 / 30.0)
		unlock_steps += 1
	await process_frame
	await _capture("02_todos_os_tiers_desbloqueados", main)
	...
	if not main.buildings.list.is_empty():
		...
		while steps < 6000 and (main.workers.list.size() < staffable.size() or not main.workers.list.all(func(w): return w.state == Worker.State.WORKING)):
			...
			steps += 1
		await process_frame
		await _capture("03_trabalhadores_nos_postos", main)
	...
	while steps < 6000 and not delivered:
		...
		for resource in main.buildings.stock:
			if main.buildings.stock[resource] > before.get(resource, 0.0):
				delivered = true
	await process_frame
	await _capture("04_carregador_entregando", main)
	...
	await process_frame
	await _capture("05_processamento", main)
	...
	await process_frame
	await _capture("06_minerio_e_lingote", main)
	...
	await process_frame
	await _capture("07_fazenda_e_comida", main)

	# Fase 7: afasta a câmera pra ver a vila inteira — a esta altura o nível
	# já passou de 4 fazia tempo (precisou pra desbloquear o tier 4 lá em
	# cima), então o alcance de exploração (raio da névoa) já está bem maior
	# que o do nível 1.
	main.camera.zoom = Vector2(0.9, 0.9)
	main.camera.position = Vector2(main._village_cell) * float(main.CELL)
	await process_frame
	await _capture("08_vila_sobe_de_nivel", main)
	...
	await process_frame
	await _capture("09_depois_de_explorar", main)
	...
	await process_frame
	await _capture("10_vista_afastada", main)
